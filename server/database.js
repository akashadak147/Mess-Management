...
function initSchema() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS billing (
      ...
      monthly_fee REAL DEFAULT 700.0, -- Fixed ₹700 monthly mess fee due by 15th
  ...
  try { db.exec("ALTER TABLE users ADD COLUMN is_owner INTEGER DEFAULT 0"); } catch(e) {}
  try { db.exec("ALTER TABLE users ADD COLUMN permissions TEXT DEFAULT 'all'"); } catch(e) {}
  try { db.exec("ALTER TABLE billing ADD COLUMN monthly_fee REAL DEFAULT 700.0"); } catch(e) {}
  try { db.exec("ALTER TABLE mess_settings ADD COLUMN monthly_fee REAL DEFAULT 700.0"); } catch(e) {}
  try { db.exec("ALTER TABLE mess_settings ADD COLUMN scanner_name TEXT DEFAULT 'Bhabani Payment Scanner'"); } catch(e) {}
  try { db.exec("ALTER TABLE mess_settings ADD COLUMN scanner_image TEXT DEFAULT '/assets/bhabani_scanner.jpeg'"); } catch(e) {}
  try { db.exec("ALTER TABLE payments ADD COLUMN payer_upi_id TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE payments ADD COLUMN upi_app TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE weekly_menu ADD COLUMN bengali_day TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE weekly_menu ADD COLUMN market_duty TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE weekly_menu ADD COLUMN special_note TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE mess_settings ADD COLUMN sms_provider TEXT DEFAULT 'fast2sms'"); } catch(e) {}
  try { db.exec("ALTER TABLE mess_settings ADD COLUMN sms_api_key TEXT"); } catch(e) {}
  try { db.exec("ALTER TABLE mess_settings ADD COLUMN sms_sender_id TEXT"); } catch(e) {}

  // Ensure default settings exist and update to Mess Mate and Bhabani scanner
  const settingsCheck = db.prepare('SELECT id FROM mess_settings WHERE id = 1').get();
  if (!settingsCheck) {
    db.prepare(`
  ...
  // Populate handwritten Mess Food & Market Duty Routine
  const handwrittenSchedule = [
    {
      day_of_week: 'Monday',
      bengali_day: 'সোমবার',
      market_duty: 'Souvik + Arnab + Ankur',
      morning_menu: 'Veg + Dal (সবজি + ডাল)',
      night_menu: 'Chana-Ponir + Dal (ছানা পনির + ডাল)',
      special_note: ''
    },
    {
      day_of_week: 'Tuesday',
      bengali_day: 'মঙ্গলবার',
      market_duty: 'Akash + Aritra + Tuhin',
      morning_menu: 'Soyabean + Dal (সয়াবিন + ডাল)',
      night_menu: 'Eggs (ডিম)',
      special_note: ''
    },
    {
      ...
      bengali_day: 'বুধবার',
      market_duty: 'Kalyan + Sujan + Arghya',
      morning_menu: 'Veg + Dal (সবজি + ডাল)',
      night_menu: 'Chicken (মাংস)',
      special_note: ''
    },
    {
      day_of_week: 'Thursday',
      bengali_day: 'বৃহস্পতিবার',
      market_duty: 'Amit + Soumen + Arijit',
      morning_menu: 'Eggs + Dal (ডিম + ডাল)',
      night_menu: 'Chana-Ponir + Dal (ছানা পনির + ডাল)',
      special_note: ''
    },
    {
      day_of_week: 'Friday',
      bengali_day: 'শুক্রবার',
      market_duty: 'Santanu + Suprovat',
      morning_menu: 'Veg + Dal (সবজি + ডাল)',
      night_menu: '(Kichuri + Potato) OR Eggs (খিচুড়ি + আলু অথবা ডিম)',
      special_note: ''
    },
    {
      day_of_week: 'Saturday',
      bengali_day: 'শনিবার',
      market_duty: 'Monodeep + B.P.G',
      morning_menu: 'Soyabean + Dal (সয়াবিন + ডাল)',
      night_menu: 'Fish or Chicken (মাছ বা মাংস)',
      special_note: ''
    },
    {
      day_of_week: 'Sunday',
      bengali_day: 'রবিবার',
      market_duty: 'Pradip + Kinshuk + Shibom + D.K.',
      morning_menu: 'Veg + Dal (সবজি + ডাল)',
      night_menu: 'Eggs + Dal (ডিম + ডাল)',
      special_note: ''
    }
  ];
  ...
  const transMenu = db.transaction(() => {
    for (const item of handwrittenSchedule) {
      upsertMenuStmt.run(item);
    }
  });
  transMenu();
 ...
 * Incorporates:
 * 1. Meal attendance count * meal_rate
 * 2. Previous month unpaid due
 * 3. Rannar Masi fixed fee (₹400)
 * 4. Late fine (₹100 if current date > 15th of the month and bill is unpaid)
 */
function recalculateAllBills(targetMonth) {
  const settings = db.prepare('SELECT * FROM mess_settings WHERE id = 1').get();
  const users = db.prepare("SELECT id, name FROM users WHERE role = 'user' AND status = 'active'").all();
  ...
  const getMealsCount = db.prepare(`
    SELECT COALESCE(SUM(morning + night), 0) as total_meals
    FROM meals
    WHERE user_id = ? AND date LIKE ?
  `);

  const getPrevBilling = db.prepare(`
    SELECT (total_payable - paid_amount) as due
    FROM billing
    WHERE user_id = ? AND month = ?
  `);

  const getExistingBilling = db.prepare(`
  ...
  const trans = db.transaction(() => {
    for (const u of users) {
      const mealRes = getMealsCount.get(u.id, `${targetMonth}-%`);
      const mealsCount = mealRes ? mealRes.total_meals : 0;
      const mealCost = 0; // No per-meal charges! Food is covered by fixed ₹700 monthly fee.
      const monthlyFee = settings.monthly_fee || 700.0; // ₹700 monthly mess fee due by 15th

      // Previous month unpaid due
      const prevBill = getPrevBilling.get(u.id, prevMonthStr);
      const prevDue = prevBill ? Math.max(0, prevBill.due) : 0;

      // Existing bill data to preserve payments made
      const existing = getExistingBilling.get(u.id, targetMonth);
      const paidAmount = existing ? existing.paid_amount : 0;
      const masiPaid = existing ? existing.masi_paid : (paidAmount >= settings.masi_fee ? 1 : 0);

      // Late fine check: if bill unpaid and day > fine_cutoff_day (15th)
      let fineAmount = 0;
      let fineApplied = 0;
      const basePayableWithoutFine = monthlyFee + prevDue + settings.masi_fee;

      if (isCurrentMonth && currentDay > settings.fine_cutoff_day) {
        if (paidAmount < basePayableWithoutFine) {
          fineAmount = settings.fine_amount; // ₹100
          fineApplied = 1;
        }
      } else if (!isCurrentMonth && new Date(year, month - 1, 1) < new Date(today.getFullYear(), today.getMonth(), 1)) {
        // For past months that were not fully paid
        if (paidAmount < basePayableWithoutFine) {
      ...
      if (paidAmount >= totalPayable && totalPayable > 0) {
        status = 'PAID';
      } else if (fineApplied === 1 || (isCurrentMonth && currentDay > settings.fine_cutoff_day)) {
        status = 'OVERDUE';
      }
 ...
 */
function logAudit(adminId, adminName, action, targetUserId = null, targetUserName = null, details = null) {
  try {
    db.prepare(`
      INSERT INTO audit_logs (admin_id, admin_name, action, target_user_id, target_user_name, details)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(adminId || null, adminName || 'System Admin', action, targetUserId || null, targetUserName || null, details || null);
  } catch (err) {
